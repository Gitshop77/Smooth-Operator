...
const DIST_ENTRY = "dist/open-cowork-mcp.mjs";
const DIST_MAP = "dist/open-cowork-mcp.mjs.map";
...
async function main() {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "open-cowork-package-smoke-"));
  try {
    ...
    if (install) {
      const installDirectory = await mkdtemp(join(tmpdir(), "open-cowork-package-install-"));
      try {
        ...
        const binary = join(installDirectory, "node_modules", ".bin", process.platform === "win32" ? "open-cowork-mcp.cmd" : "open-cowork-mcp");
        ...
        const transport = new StdioClientTransport({
          ...
          env: {
            ...
            OPEN_COWORK_DATA_DIR: join(installDirectory, "data"),
            OPEN_COWORK_LOG_LEVEL: "error",
          },
          stderr: "pipe",
        });
        const client = new Client({ name: "open-cowork-package-smoke", version: "1.0.0" }, { versionNegotiation: { mode: "auto" } });
        try {
...
async function verifySourceMetadata(packageJson) {
  if (packageJson.name !== "open-cowork-mcp") {
    throw new Error("package.json name must remain open-cowork-mcp.");
  }
  ...
  if (packageJson.bin?.["open-cowork-mcp"] !== DIST_ENTRY) {
    throw new Error(`package.json must expose open-cowork-mcp at ${DIST_ENTRY}.`);
  }
...
async function verifySourcemap() {
  ...
  if (!entry.includes("//# sourceMappingURL=open-cowork-mcp.mjs.map")) {
