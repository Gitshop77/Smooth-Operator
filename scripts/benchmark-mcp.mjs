...
const DIST_SERVER_ENTRY = join(ROOT, "dist", "open-cowork-mcp.mjs");
const FIXTURE_HTML = `<!doctype html><html><head><title>Open Cowork benchmark fixture</title></head><body><main id="benchmark-fixture"><h1>Benchmark fixture</h1><p>Deterministic browser evidence for MCP measurements.</p><button id="fixture-button">Continue</button><input aria-label="Fixture input" value="ready"></main></body></html>`;
...
fail-closed error paths. --live requires OPEN_COWORK_BENCHMARK_BROWSER_EXECUTABLE
...
function benchmarkEnvironment(options, dataDirectory, userDataDirectory) {
  ...
  environment.OPEN_COWORK_TRANSPORT = "stdio";
  environment.OPEN_COWORK_BROWSER_MODE = options.live ? "launch" : "disabled";
  environment.OPEN_COWORK_DATA_DIR = dataDirectory;
  environment.OPEN_COWORK_LOG_LEVEL = "error";
  if (options.live) {
    environment.OPEN_COWORK_BROWSER_EXECUTABLE = options.executable;
    environment.OPEN_COWORK_BROWSER_USER_DATA_DIR = userDataDirectory;
    environment.OPEN_COWORK_BROWSER_HEADLESS = "true";
  }
  return environment;
}
...
async function runIteration(options, fixtureUrl) {
  const dataDirectory = await mkdtemp(join(tmpdir(), "open-cowork-mcp-benchmark-"));
  ...
  const client = new Client({ name: "open-cowork-benchmark", version: "1.0.0" }, { versionNegotiation: { mode: "auto" } });
  let connected = false;
  try {
...
async function main() {
  ...
  if (options.dist) {
    await access(DIST_SERVER_ENTRY).catch(() => {
      throw new Error("--dist requires dist/open-cowork-mcp.mjs. Run npm run build first.");
    });
  }
  if (options.live) {
    options.executable = process.env.OPEN_COWORK_BENCHMARK_BROWSER_EXECUTABLE ?? process.env.OPEN_COWORK_BROWSER_EXECUTABLE;
    if (!options.executable) {
      throw new Error("--live requires OPEN_COWORK_BENCHMARK_BROWSER_EXECUTABLE.");
    }
  }
