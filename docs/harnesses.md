...
Open Cowork speaks MCP over stdio. The `open-cowork-mcp install <target>`
...
```text
/absolute/path/to/node /absolute/path/to/dist/open-cowork-mcp.mjs
```
...
```sh
claude mcp add --scope user open-cowork -- open-cowork-mcp
```
...
form. Passing `opencode mcp add open-cowork -- open-cowork-mcp` is not a stable
...
```sh
open-cowork-mcp install opencode
```
...
```sh
copilot mcp add open-cowork -- open-cowork-mcp
copilot mcp get open-cowork
copilot mcp list
```
...
```sh
codex mcp add open-cowork -- open-cowork-mcp
codex mcp get open-cowork
codex mcp list
```
...
```sh
gemini mcp add open-cowork open-cowork-mcp --scope user
gemini mcp list
```
...
```sh
code --add-mcp '{"name":"open-cowork","command":"open-cowork-mcp","args":[]}'
```
...
`open-cowork` entry, and refuses symlinked config files/directories. For an
...
```sh
claude mcp remove open-cowork
copilot mcp remove open-cowork
codex mcp remove open-cowork
gemini mcp remove open-cowork
```

For JSON clients, remove only the `open-cowork` property and leave unrelated
...
- **Existing config is rejected:** inspect the file for malformed JSON,
  ...
  `open-cowork` entry. The fail-closed behavior protects unrelated settings.
- **No tools after installation:** close and restart the harness, then list
  its MCP servers. Run `open-cowork-mcp --version` directly to verify the
