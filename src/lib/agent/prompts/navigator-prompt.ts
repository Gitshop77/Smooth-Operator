 ...
 * preserved VERBATIM; the prose sections have converged to the same terse
 * forms, so the two variants differ only in the retained intro/heading prose
 * (measured sizes pinned in tests/compact-prompt.test.ts).
...
${evaluateGuidance(mode)}`;
  ...
  if (compact) {
    ...
    // behavior block below is byte-identical to the full prompt, and the
    // prose sections use the same terse forms (the full branch converged to
    // them). Kept as a single template so the security-critical blocks cannot
