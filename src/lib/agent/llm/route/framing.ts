...
/**
 * Build a stateful SSE framer that buffers `data:` lines and emits one frame
 * per SSE event (on the blank-line boundary), concatenating multi-line `data:`
 * fields with "\n".
 *
 * The instance is stateful, so callers must use a FRESH instance per stream.
 * `sse` (below) satisfies this by constructing a new framer on every `parse`
 * call, which makes it safe to drive many concurrent streams — each `parse`
 * invocation gets its own isolated `dataLines` buffer.
 */
...
export const sse: Framing = {
  parse: (chunk: string): Frame[] => createSSEFramer().parse(chunk),
};
