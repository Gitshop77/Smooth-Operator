...
export async function withLLMRetry<T>(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      // Cancelled/aborted — propagate immediately, no retry.
      ...
      const is429 = status === 429 || msg.includes("429") || /too many requests/i.test(msg);
      const is5xx = (typeof status === "number" && status >= 500 && status < 600) || /\b5\d\d\b/.test(msg);
      const isNetwork = /fetch|network|econn|timeout/i.test(msg);
