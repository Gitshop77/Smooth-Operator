  ...
  try {
    ...
    try {
      raw = await Promise.race([
        chrome.runtime
          .sendMessage({ type: "SAVE_AS_PDF", fileName: pdfName, action, ...(ctx.dispatchToken ? { token: ctx.dispatchToken } : {}), ...(ctx.effectCapability ? { effectCapability: ctx.effectCapability } : {}) })
