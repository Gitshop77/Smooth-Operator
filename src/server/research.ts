...
export class ResearchService {
  ...
  async research(query: string, options: { maxResults?: number; maxChars?: number } = {}, signal?: AbortSignal): Promise<ResearchResult> {
    ...
    // maxChars is one aggregate budget for the textual fields returned by the
    // search, not a per-result multiplier. URL metadata and the fixed
    // untrusted-data wrapper markers are intentionally outside this budget.
