...
function normalizeHost(host: string): string {
  ...
  // URL.hostname is already ASCII for most callers, but configured allow and
  // block patterns may be Unicode.  Canonicalize both sides to the same
  // IDNA representation before suffix matching.  IP literals are handled
  // separately because domainToASCII intentionally rejects colon notation.
  if (isIP(trimmed)) {
...
function isPrivateIpv6(host: string): boolean {
  ...
  const mappedIpv4 = embeddedIpv4
    ?? (parts.slice(0, 5).every((part) => part === 0) && (parts[5] === 0 || parts[5] === 0xffff)
