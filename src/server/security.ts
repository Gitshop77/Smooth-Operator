...
export function wrapUntrustedText(label: string, value: string, maxChars = DEFAULT_UNTRUSTED_LIMIT): string {
  ...
  const endTagPattern = new RegExp(`<\\/untrusted_${safeLabel}\\s*>`, "gi");
  const normalized = normalizeUntrustedText(value.slice(0, limit))
    .replace(endTagPattern, "[UNTRUSTED_END_TAG_TEXT]")
    .slice(0, limit);
