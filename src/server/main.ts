...
const INSTALL_USAGE = `Usage: open-cowork-mcp install <${supportedHarnessTargets().join("|")}>
       open-cowork-mcp install --help`;

const HELP = `Open Cowork MCP server

Usage:
  open-cowork-mcp [--transport stdio|http] [--config path]
  npm start -- [--transport stdio|http] [--config path]
  open-cowork-mcp --version
  open-cowork-mcp install <harness>
  open-cowork-mcp install --help

Environment:
  OPEN_COWORK_TRANSPORT=stdio|http
  OPEN_COWORK_BROWSER_MODE=disabled|connect|launch|managed
  OPEN_COWORK_BROWSER_WS_ENDPOINT=ws://...
  OPEN_COWORK_BROWSER_URL=http://127.0.0.1:9222
  OPEN_COWORK_BROWSER_EXECUTABLE=/path/to/chrome
  OPEN_COWORK_BROWSER_CONNECT_TIMEOUT_MS=30000
  OPEN_COWORK_BROWSER_CDP_TIMEOUT_MS=30000
  OPEN_COWORK_ALLOWED_DOMAINS=example.com,*.example.org
  OPEN_COWORK_ALLOW_EVAL=true (explicit page-JavaScript opt-in)
  OPEN_COWORK_HTTP_TOKEN=... (required for remote HTTP)
  OPEN_COWORK_HTTP_MAX_BODY_BYTES=2000000
`;

const MAX_HTTP_CONCURRENCY = 32;
// Keep long-lived SSE/subscription exchanges in their own small pool so they
// cannot consume every ordinary request slot.  The pool is deliberately
// bounded; this is not an unbounded streaming bypass.
...
export async function main(args = process.argv.slice(2)): Promise<void> {
  if (args[0] === "install") {
    ...
    process.stdout.write(`${await installHarness(target)}\n`);
    return;
  }
  ...
  // serveStdio owns the SDK transport and closes its pinned server when the
  // input stream ends, but the application-owned runtime needs the same
  // lifecycle signal so an owned Chromium process cannot keep this process
  // alive after an MCP client disconnects.
