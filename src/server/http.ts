...
export const HTTP_HANDLER_SHUTDOWN_TIMEOUT_MS = 5_000;
...
export async function serveHttp(runtime: ServerRuntime, shutdown: (reason: string) => Promise<void>): Promise<void> {
...
export function observeShutdown(promise: Promise<void>, runtime: ServerRuntime): void {
  void promise.then(
    () => {
      process.exitCode = 0;
    },
...
export async function runBoundedShutdownPhase(
  label: string,
