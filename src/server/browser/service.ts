...
export class BrowserService {
  ...
  async execute(action: BrowserAction, signal?: AbortSignal): Promise<unknown> {
    ...
    return this.withOperationLock(signal, (operationSignal) => this.executeUnlocked(action, operationSignal), timeoutMs, timeoutMs);
  }
    ...
    try {
      if (this.config.browser.mode === "managed") {
        ...
        if (endpoint.state === "live" && endpoint.browserURL) {
          ...
          if (!executablePath) {
            throw new AppError("BROWSER_NOT_CONFIGURED", `Managed browser mode could not find Chrome. Checked: ${chromeExecutableSearchPaths().join(", ")}. Install Chrome or set OPEN_COWORK_BROWSER_EXECUTABLE.`);
          }
        ...
        if (!this.config.browser.executablePath) {
          throw new AppError("BROWSER_NOT_CONFIGURED", "Launch mode requires OPEN_COWORK_BROWSER_EXECUTABLE.");
        }
        ownsBrowser = true;
        connection = this.launch({
          headless: this.config.browser.headless,
  ...
  /**
   * Puppeteer resumes an auto-attached target after its own target setup. A
   * page can issue its first request in that small window, before the page
   * object (and our normal request listener) exists. Install a Fetch guard at
   * the CDP session boundary so every new target is policy-checked while it is
   * still paused. The guard is removed only after configurePage has installed
   * the regular Puppeteer interception listener.
   */
  ...
  private async accessibilitySnapshot(state: PageState, maxNodes: number, maxChars: number, interestingOnly: boolean): Promise<unknown> {
    ...
    try {
      ...
      return {
        pageId: state.id,
        snapshotId: state.snapshotId ?? randomUUID(),
  ...
  /**
   * AbortSignal is advisory in Puppeteer.  If a protocol operation ignores it,
   * reset the active browser/session immediately so a later operation cannot
   * keep using a potentially-mutated target.  The operation queue node is
   * deliberately held until the original promise settles; returning a timeout
   * to the caller must never allow browser work to overlap.
   */
