...
export class ServerRuntime {
  ...
  private constructor(readonly config: ServerConfig, private readonly browserProfileLease?: BrowserProfileLease) {
    this.logger = new Logger(config.logLevel, { component: "open-cowork-mcp" });
    ...
    if (!this.closePromise) {
      this.closePromise = (async () => {
        ...
        if (browserClose.status === "complete" && browserOutcome?.succeeded !== false) {
          ...
          // An owned browser may still be using this profile after a close
          // failure or timeout. Retain the lock for explicit operator recovery
          // instead of allowing another process to launch into a live profile.
  ...
  publicCapabilities(): Record<string, unknown> {
    ...
    return {
      ...
      server: { name: "open-cowork", version: SERVER_VERSION },
...
const BROWSER_PROFILE_LOCK_NAME = ".open-cowork-profile.lock";
...
async function acquireBrowserProfileLease(profileDirectory: string): Promise<BrowserProfileLease> {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (existing === "active") {
        throw new AppError("BROWSER_PROFILE_IN_USE", "The native browser profile is already in use by another Open Cowork process.", { retryable: true });
      }
      if (existing === "unknown") {
        throw new AppError("BROWSER_PROFILE_LOCK_FAILED", "The native browser profile has an unreadable lock. Verify that no Open Cowork process is using it, then remove the lock file.", { retryable: true });
      }
      if (existing === "stale") {
        ...
        throw new AppError("BROWSER_PROFILE_LOCK_FAILED", "The native browser profile has a stale lock. Verify that no Open Cowork process is using it, then remove the lock file and retry.", { retryable: true });
      }
    }
  }
...
async function ensurePrivateDirectory(path: string): Promise<void> {
  ...
  // Never use mkdir({recursive:true}) here: it follows a symlink in an
  // ancestor before we get a chance to inspect it.  Walk to the nearest
  // existing ancestor first, reject every non-system symlink, then create one
  // component at a time and inspect each newly-created component.
