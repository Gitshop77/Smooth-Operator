...
export const SnapshotRequestSchema = z.object({
  ...
  if (input.fullPage !== undefined && input.full !== undefined) {
    context.addIssue({ code: "custom", message: "Provide fullPage or full, not both." });
  }
  if (input.maxDimension !== undefined && input.max_dim !== undefined) {
    context.addIssue({ code: "custom", message: "Provide maxDimension or max_dim, not both." });
  }
...
export const EvaluateRequestSchema = z.object({ code: z.string().trim().min(1).max(40_000), ...PageInput }).strict();
