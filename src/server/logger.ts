...
/**
 * Project an object key to the bounded form used in redacted output while
 * retaining every distinct source key.  Long keys can share the same prefix,
 * so a plain truncation is not sufficient: a later assignment would silently
 * replace an earlier value.  Collision suffixes are deterministic and remain
 * bounded by the collection limit.
 */
