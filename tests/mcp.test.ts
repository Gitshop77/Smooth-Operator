...
describe("native MCP registry", () => {
  it("completes a real MCP handshake and exposes only native server capabilities", async () => {
    ...
    expect(resources.resources.map((resource) => resource.uri)).toContain("open-cowork://server/capabilities");
    expect(resourceTemplates.resourceTemplates.map((resource) => resource.uriTemplate)).toContain("open-cowork://browser/page/{pageId}");
    ...
    expect(toolByName.get("browser_find_text")?.annotations?.readOnlyHint).not.toBe(true);
    ...
    const capabilityResource = await client.readResource({ uri: "open-cowork://server/capabilities" });
    ...
    const calls: Array<[string, Record<string, unknown>]> = [
      ...
      ["browser_upload", { selector: "input[type=file]", filePath: "/tmp/open-cowork-test/file.txt" }],
      ["browser_screenshot", {}], ["browser_pdf", { outputPath: "/tmp/open-cowork-test/page.pdf" }], ["browser_downloads", {}],
  ...
  it("forwards the MCP request signal to live resource operations", async () => {
    ...
    try {
      ...
      for (const uri of [
        ...
        "open-cowork://browser/logs/network",
        "open-cowork://browser/logs/console",
        "open-cowork://browser/page/page-1",
      ]) {
  ...
  it("does not expose runtime exception text from resource failures", async () => {
    ...
    try {
      await Promise.all([server.connect(serverTransport), client.connect(clientTransport)]);
      const error = await client.readResource({ uri: "open-cowork://browser/page/current" }, { cacheMode: "bypass" }).then(
        () => undefined,
