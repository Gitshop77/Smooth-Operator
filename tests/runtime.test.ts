...
describe("runtime lifecycle", () => {
  ...
  it("creates and protects the owned browser profile before launch", async () => {
    const directory = await mkdtemp(join(tmpdir(), "open-cowork-runtime-profile-"));
  ...
  it("leases the native profile and rejects concurrent owners", async () => {
    const directory = await mkdtemp(join(tmpdir(), "open-cowork-runtime-lock-"));
    const profile = join(directory, "browser");
    const base = testConfig();
    ...
    await expect(access(join(profile, ".open-cowork-profile.lock"))).rejects.toMatchObject({ code: "ENOENT" });
    await rm(directory, { recursive: true, force: true });
  });

  it("leases the managed browser profile before attempting reattachment", async () => {
    const directory = await mkdtemp(join(tmpdir(), "open-cowork-runtime-managed-lock-"));
    const profile = join(directory, "browser");
    const base = testConfig();
    const config = testConfig({
      dataDir: directory,
  ...
  it("fails closed on stale profile locks instead of reclaiming them racy", async () => {
    const directory = await mkdtemp(join(tmpdir(), "open-cowork-runtime-stale-lock-"));
    const profile = join(directory, "browser");
    const base = testConfig();
    ...
    await writeFile(join(profile, ".open-cowork-profile.lock"), JSON.stringify({ pid: 2_147_483_647, token: "stale" }));
    await expect(ServerRuntime.create(config)).rejects.toMatchObject({ code: "BROWSER_PROFILE_LOCK_FAILED" });
    await expect(access(join(profile, ".open-cowork-profile.lock"))).resolves.toBeUndefined();
    await rm(directory, { recursive: true, force: true });
  });

  it("retains an owned profile lease when browser shutdown fails", async () => {
    const directory = await mkdtemp(join(tmpdir(), "open-cowork-runtime-close-failure-"));
    const profile = join(directory, "browser");
    const base = testConfig();
    const runtime = await ServerRuntime.create(testConfig({
      dataDir: directory,
  ...
  it("rejects a symlinked owned browser userDataDir before acquiring its lease", async () => {
    const directory = await mkdtemp(join(tmpdir(), "open-cowork-runtime-profile-symlink-"));
    ...
    await expect(access(join(physicalProfile, ".open-cowork-profile.lock"))).rejects.toMatchObject({ code: "ENOENT" });
    await rm(directory, { recursive: true, force: true });
  });
});
