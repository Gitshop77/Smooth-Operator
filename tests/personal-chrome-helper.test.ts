...
describe("personal chrome helper", () => {
  it("spawns Chrome and probes 9222 until live", async () => {
    ...
    const result = await launchPersonalChrome({ executablePath: "/usr/bin/google-chrome", dataDir: "/tmp/test-home/.smooth-operator", port: 9222, spawn: mockSpawn, probe: mockProbe });
    expect(mockSpawn).toHaveBeenCalledWith(expect.stringContaining("chrome"), expect.arrayContaining(["--remote-debugging-port=9222"]), expect.objectContaining({ detached: true }));
  ...
  it("surfaces a connect-timeout error when the endpoint never becomes live", async () => {
    ...
    await expect(launchPersonalChrome({
      executablePath: "/usr/bin/google-chrome", dataDir: "/tmp/test-home/.smooth-operator",
