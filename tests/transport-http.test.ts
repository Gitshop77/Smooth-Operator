...
describe("HTTP transport", () => {
  it("enforces auth and origin checks before completing a valid MCP initialize", async () => {
    ...
    const dataDir = await mkdtemp(join(tmpdir(), "open-cowork-http-"));
    ...
    const child = spawn(process.execPath, [join(process.cwd(), "node_modules", "tsx", "dist", "cli.mjs"), "src/server/main.ts", "--transport", "http"], {
      ...
      env: {
        ...
        OPEN_COWORK_HTTP_PORT: String(port),
        OPEN_COWORK_HTTP_TOKEN: token,
        OPEN_COWORK_HTTP_MAX_BODY_BYTES: "1024",
    ...
    expect(await valid.text()).toContain('"name":"open-cowork"');
