...
const SOURCE_ENTRY: HarnessCommand = { command: "open-cowork-mcp", args: [] };
...
describe("harness installer", () => {
  it("plans current non-interactive CLI argv without launching programs", async () => {
    const entry = { command: "/opt/Node Runtime/node", args: ["/opt/Open Cowork/dist/open-cowork-mcp.mjs"] };
    ...
    expect(calls).toEqual([
      { command: "claude", args: ["mcp", "add", "--scope", "user", "open-cowork", "--", "open-cowork-mcp"] },
      { command: "copilot", args: ["mcp", "add", "open-cowork", "--", "open-cowork-mcp"] },
      { command: "codex", args: ["mcp", "add", "open-cowork", "--", "open-cowork-mcp"] },
      { command: "gemini", args: ["mcp", "add", "open-cowork", "open-cowork-mcp", "--scope", "user"] },
      { command: "code", args: ["--add-mcp", JSON.stringify({ name: "open-cowork", command: entry.command, args: entry.args })] },
    ]);
  });

  it("uses a supported direct config path for OpenCode instead of its interactive add command", async () => {
    const directory = await makeDirectory("open-cowork-installer-opencode-");
    const path = join(directory, "opencode.json");
    const entry = { command: "/usr/local/bin/node", args: ["/opt/open-cowork/dist/open-cowork-mcp.mjs"] };
    try {
      ...
      expect(installed.mcp.servers["open-cowork"]).toEqual({ type: "local", command: [entry.command, ...entry.args] });
    } finally {
  ...
  it("uses structured absolute executable fields for GUI JSON clients", async () => {
    const directory = await makeDirectory("open-cowork-installer-gui-");
    const configPath = join(directory, "cursor.json");
    const entry = { command: "/Applications/Node Runtime/bin/node", args: ["/Applications/Open Cowork/dist/open-cowork-mcp.mjs"] };
    try {
      ...
      expect(installed.mcpServers["open-cowork"]).toEqual(entry);
      expect(installed.mcpServers["open-cowork"].args).toEqual(["/Applications/Open Cowork/dist/open-cowork-mcp.mjs"]);
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  it("preserves URLs while parsing JSONC and secures an exclusive config backup", async () => {
    const directory = await makeDirectory("open-cowork-installer-jsonc-");
    ...
    try {
      ...
      expect(installed.mcpServers["open-cowork"]).toEqual({ command: "open-cowork-mcp", args: [] });
  ...
  it("fails closed for a non-object mcpServers value without changing the file", async () => {
    const directory = await makeDirectory("open-cowork-installer-invalid-");
  ...
  it("fails closed for a conflicting existing server and preserves unrelated data", async () => {
    const directory = await makeDirectory("open-cowork-installer-conflict-");
    const configPath = join(directory, "config.json");
    const original = JSON.stringify({ unrelated: { keep: true }, mcpServers: { "open-cowork": { command: "other", args: ["--unsafe"] } } });
    try {
      await writeFile(configPath, original);
  ...
  it("is idempotent and does not replace the first backup on a repeat install", async () => {
    const directory = await makeDirectory("open-cowork-installer-repeat-");
    const configPath = join(directory, "config.json");
    try {
  ...
  it("supports the current and legacy OpenCode config shapes without replacing unrelated values", async () => {
    const directory = await makeDirectory("open-cowork-installer-opencode-shapes-");
    const modernPath = join(directory, "modern.json");
    const legacyPath = join(directory, "legacy.json");
    const entry = { command: "/opt/node", args: ["/opt/open-cowork.mjs"] };
    try {
      ...
      expect(modern.mcp.servers["open-cowork"]).toEqual({ type: "local", command: [entry.command, ...entry.args] });
      ...
      expect(legacy.mcp["open-cowork"]).toEqual({ type: "local", command: [entry.command, ...entry.args], enabled: true });
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  it("treats disabled matching OpenCode entries as conflicts instead of idempotent", async () => {
    const directory = await makeDirectory("open-cowork-installer-opencode-disabled-");
    const modernPath = join(directory, "modern.json");
    const legacyPath = join(directory, "legacy.json");
    const entry = { command: "/opt/node", args: ["/opt/open-cowork.mjs"] };
    try {
      await writeFile(modernPath, JSON.stringify({ mcp: { servers: { "open-cowork": { type: "local", command: [entry.command, ...entry.args], disabled: true } } } }));
      ...
      expect(JSON.parse(await readFile(modernPath, "utf8"))).toEqual({ mcp: { servers: { "open-cowork": { type: "local", command: [entry.command, ...entry.args], disabled: true } } } });

      await writeFile(legacyPath, JSON.stringify({ mcp: { "open-cowork": { type: "local", command: [entry.command, ...entry.args], enabled: false } } }));
      ...
      expect(JSON.parse(await readFile(legacyPath, "utf8"))).toEqual({ mcp: { "open-cowork": { type: "local", command: [entry.command, ...entry.args], enabled: false } } });
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  it("treats absent or explicitly enabled OpenCode entries as idempotent", async () => {
    const directory = await makeDirectory("open-cowork-installer-opencode-enabled-");
    const modernPath = join(directory, "modern.json");
    const legacyPath = join(directory, "legacy.json");
    const entry = { command: "/opt/node", args: ["/opt/open-cowork.mjs"] };
    try {
      await writeFile(modernPath, JSON.stringify({ mcp: { servers: { "open-cowork": { type: "local", command: [entry.command, ...entry.args], disabled: false } } } }));
      ...
      await writeFile(legacyPath, JSON.stringify({ mcp: { "open-cowork": { type: "local", command: [entry.command, ...entry.args] } } }));
  ...
  it("honors an explicit OpenCode config path without switching to a sibling JSONC file", async () => {
    const directory = await makeDirectory("open-cowork-installer-opencode-explicit-");
    ...
    try {
      ...
      expect(JSON.parse(await readFile(explicitPath, "utf8"))).toMatchObject({ mcp: { servers: { "open-cowork": { type: "local", command: [entry.command, ...entry.args] } } } });
  ...
  it("prefers an existing JSONC file only for implicit OpenCode resolution", async () => {
    const directory = await makeDirectory("open-cowork-installer-opencode-implicit-");
    ...
    try {
      ...
      expect(JSON.parse(await readFile(jsoncPath, "utf8"))).toMatchObject({ mcp: { servers: { "open-cowork": { type: "local", command: [entry.command, ...entry.args] } } } });
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  it("rejects malformed OpenCode server collections without replacing the file", async () => {
    const directory = await makeDirectory("open-cowork-installer-opencode-invalid-");
  ...
  it("does not follow a pre-existing configuration-backup symlink", async () => {
    const directory = await makeDirectory("open-cowork-installer-backup-");
    const configPath = join(directory, "config.json");
  ...
  it("rejects a symlinked configuration file before parsing or writing", async () => {
    const directory = await makeDirectory("open-cowork-installer-config-link-");
  ...
  it("rejects a symlinked configuration directory before mkdir can follow it", async () => {
    const directory = await makeDirectory("open-cowork-installer-directory-link-");
