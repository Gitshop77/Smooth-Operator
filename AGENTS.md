...
.github/
  workflows/ci.yml          Two jobs, both on **Node 22** (bumped from 20.18 because Prisma 7.8.0 / `@prisma/streams-local` hard-require ≥22; the runner's default 20.18 failed `npm ci` with "Prisma only supports Node.js versions 20.19+, 22.12+, 24.0+").
    - **test** (root): `npm ci` → install **mini-services/cowork-events** deps (a separate `npm ci` in that dir) → `npm run lint` → `npx tsc --noEmit` (root type-check; `tests/cowork-events.test.ts` imports the mini-service, so its `socket.io`/`z-ai-web-dev-sdk` deps must be installed *before* this step or it fails with "Cannot find module") → `npx vitest run --coverage` → `npm run build:extension` + static-file sync check → `npm audit --audit-level=high && npm audit signatures` (root + mini-service).
    - **cockpit**: `npm ci` → `npm audit --audit-level=high && npm audit signatures` → `npm run db:generate` → `npm run db:apply` → `npx tsc --noEmit` → `npm run lint` → `npm test` → `npm run build`. Sets `DATABASE_URL: file:./db/cowork.db` as a **job-level env var**: `cockpit/.env` is gitignored, and Prisma 7 moved the datasource URL out of `schema.prisma` into `prisma.config.ts`, which reads it eagerly via `env('DATABASE_URL')` at load time — so CI must inject it. (`chrome-extension/` is fully gitignored build output regenerated by `npm run build:extension`, so this job never depends on committed extension files.)
    - **mini-service type-check**: `npx tsc --noEmit -p mini-services/cowork-events/tsconfig.json` (deps installed by the test job's earlier `npm ci`).
...
- **Per-model vision detection**: `modelSupportsVision()` checks the models.dev catalog `attachment` field + name-based heuristic fallback; `buildProvider()` is async and patches `supportsVision` per model.
